...
def STRidge(X0, y, lam, maxit, tol, normalize = 2, print_results = False):
    ...
    # FIX!! DONE
    if len(biginds) > 0 and X[:, biginds].shape[1] > 0:
        w[biginds] = np.linalg.lstsq(X[:, biginds], y, rcond=None)[0]
    else:
        # Optional: Print or return zero vector if nothing survives threshold
        # print("All coefficients were thresholded out.")
        return w  # or np.zeros_like(w)
